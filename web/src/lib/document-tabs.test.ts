...
describe('getTabsForDocumentType', () => {
  ...
  it('returns default sprint tabs for sprint documents', () => {
...
describe('getTabsForDocument', () => {
  // Guards deep-linked sprint URLs from rendering the wrong workspace tabs after status changes.
  it('uses planning sprint tabs when status is planning', () => {
  ...
  it('uses active sprint tabs when status is active', () => {
    const tabs = getTabsForDocument({
      id: 'sprint-2',
      title: 'Active sprint',
      document_type: 'sprint',
