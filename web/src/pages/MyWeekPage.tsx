...
export function MyWeekPage() {
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries({ queryKey: myWeekKeys.all });
    };
  }, [queryClient]);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm text-muted px-1.5">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-orange-300">
                    Last week's retro is not complete
                  </p>
                  <p className="text-xs text-orange-300/70 mt-0.5">
                    Week {previous_retro!.week_number} retro needs your input
                  </p>
                </div>
                {previous_retro!.id ? (
                  <Link
                  ...
                  <button
                    onClick={() =>
                      handleCreateRetro(previous_retro!.week_number)
                    }
              ...
              {plan ? (
                ...
                >
                  {(() => {
                    ...
                    if (isDue) {
                      return (
                        <span className="absolute top-3 right-3 text-xs bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded">
                          Due today
                        </span>
                      );
                    }
                    if (isSubmitted) {
                      return (
                        <span className="absolute top-3 right-3 text-xs bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded">
                          Submitted
                        </span>
                      );
                    }
                    if (hasContent) {
                      return (
                        <span className="absolute top-3 right-3 text-xs bg-border text-muted px-1.5 py-0.5 rounded">
                          Unsubmitted
                        </span>
                      );
                    }
              ...
              {standups.map((slot: StandupSlot) => {
                ...
                const dateLabel = (
                  <div className="w-20 flex-shrink-0">
                    ...
                    <span className="text-xs text-muted ml-1">
                      {new Date(slot.date + "T00:00:00Z").toLocaleDateString(
                        "en-US",
                ...
                return (
                  <button
                    ...
                    className={cn(
                      rowClass,
                      "w-full text-left cursor-pointer disabled:opacity-50",
                    )}
                  >
                    {dateLabel}
                    <div className="flex-1 min-w-0">
                      <span className="text-xs text-muted">
                        {creating === `standup-${slot.date}`
                          ? "Creating..."
                          : "+ Write update"}
                      </span>
                    </div>
