...
describe('Accountability Service', () => {
  ...
  beforeEach(() => {
    ...
    mockPoolQuery.mockResolvedValue(emptyRows());
  ...
  const mockSetupQueries = (sprintStartDate = '2024-01-01') => {
    return mockPoolQuery
      // 1. Workspace query
      .mockResolvedValueOnce(mockRows([{ sprint_start_date: sprintStartDate }]))
      // 2. Person document lookup
      .mockResolvedValueOnce(mockRows([{ id: personId }]));
  };
  ...
  describe('checkMissingAccountability', () => {
    it('returns empty array when workspace not found', async () => {
      mockPoolQuery.mockResolvedValueOnce(emptyRows());
  ...
  describe('date calculations', () => {
    it('handles workspace start date as Date object', async () => {
      ...
      mockPoolQuery
        .mockResolvedValueOnce(mockRows([{ sprint_start_date: startDate }]))
        .mockResolvedValueOnce(mockRows([{ id: personId }]))
        .mockResolvedValueOnce(emptyRows())
        .mockResolvedValueOnce(emptyRows())
        .mockResolvedValueOnce(emptyRows())
  ...
  describe('per-person weekly retro due window (Thursday through Friday EOD)', () => {
    ...
    it('shows retro as due on Thursday', async () => {
      ...
      mockPoolQuery
        .mockResolvedValueOnce(mockRows([{ id: 'plan-1', content: { type: 'doc', content: [{ type: 'text', text: 'My plan' }] } }]))
  ...
  describe('next-sprint lookahead', () => {
    it('checks both current AND next sprint for accountability', async () => {
      ...
      mockSetupQueries()
        ...
        .mockResolvedValueOnce(mockRows([{ id: 'plan-1', content: { type: 'doc', content: [{ type: 'text', text: 'done' }] } }]))
        // Week 1 retro - exists (done) (today Jan 7 >= retroDueStr Jan 4)
        .mockResolvedValueOnce(mockRows([{ id: 'retro-1', content: { type: 'doc', content: [{ type: 'text', text: 'done' }] } }]))
